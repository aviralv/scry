...
describe('Registry', () => {
  ...
  it('renders People tab by default', async () => {
    ...
    renderWithRouter();
    await waitFor(() => expect(screen.getByDisplayValue('Andre')).toBeInTheDocument());
    expect(screen.queryByDisplayValue('EA')).not.toBeInTheDocument();
  });
  ...
  it('switches to Projects tab via URL', async () => {
    ...
    await waitFor(() => expect(screen.getByDisplayValue('EA')).toBeInTheDocument());
    expect(screen.queryByDisplayValue('Andre')).not.toBeInTheDocument();
  });
  ...
  it('marks a row dirty when its name changes and clears on Save', async () => {
    ...
    renderWithRouter();
    const nameInput = await waitFor(() => screen.getByDisplayValue('Andre'));
  ...
  it('renders path-scoped errors per row on 400', async () => {
    vi.mocked(api.getRegistry).mockResolvedValue({
      people: { andre: { name: 'Andre', identifiers: {} } },
      projects: {},
    });
  ...
  it('auto-expands the More fields section when an extended-field error arrives', async () => {
    vi.mocked(api.getRegistry).mockResolvedValue({
      people: { andre: { name: 'Andre', identifiers: {} } },
      projects: {},
    });
    vi.mocked(api.putRegistry).mockRejectedValue(
      new ApiCallError(400, {
        error: 'invalid-body',
        errors: [{ path: ['people', 'andre', 'identifiers', 'email'], message: 'Invalid email' }],
      }),
    );
    renderWithRouter();
    const nameInput = await waitFor(() => screen.getByDisplayValue('Andre'));
    // Dirty the row so Save is enabled.
    fireEvent.change(nameInput, { target: { value: 'Andre C' } });
    fireEvent.click(screen.getByRole('button', { name: /save changes/i }));
    // identifiers section should auto-open and surface the error
    await waitFor(() => expect(screen.getByText(/invalid email/i)).toBeInTheDocument());
  });

  it('opens add-Person modal and adds a row (auto-expanded)', async () => {
    ...
    await waitFor(() => expect(screen.getByDisplayValue('Jens')).toBeInTheDocument());
    expect(screen.getByLabelText('dirty')).toBeInTheDocument();
    // newly-added rows mount with More-fields open so identifiers are reachable
    expect(screen.getByLabelText(/hide more fields/i)).toBeInTheDocument();
  });
  ...
  it('confirms then deletes a row from the working copy', async () => {
    ...
    renderWithRouter();
    await waitFor(() => screen.getByDisplayValue('Andre'));
    fireEvent.click(screen.getByRole('button', { name: /delete andre/i }));
    await waitFor(() => expect(screen.queryByDisplayValue('Andre')).not.toBeInTheDocument());
    expect(screen.getByLabelText('dirty')).toBeInTheDocument();
  });
});
